...
void database_insert()
{
    ...
    while (1)
    {
        ...
        if (connected && uart_cond && smph_cond)
        {
            ...
            esp_http_client_set_header(http_client, "Content-Encoding", "deflate");
            esp_http_client_set_header(http_client, "Content-Length", (char *)&max_compressed_length);
            esp_http_client_set_post_field(http_client, &compressed_data, max_compressed_length);
